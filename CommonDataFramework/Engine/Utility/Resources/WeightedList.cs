...
public class WeightedList<T> : IEnumerable<T>
{
    ...
    private List<T> _list = new(); // 06.01.2024 by MarcelWRLD: removed 'readonly' modifier to allow cloning a weighted list
    private List<int> _weights = new(); // 06.01.2024 by MarcelWRLD: removed 'readonly' modifier to allow cloning a weighted list
    ...
    private void Recalculate()
    {
        ...
        {
            int l = small[small.Count - 1]; // 5.1
            small.RemoveAt(small.Count - 1);
            int g = large[large.Count - 1]; // 5.2
